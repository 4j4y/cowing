...
func main() {
	...
	for usingPin {
		callCowinUsingPin(pin, GetDate(loc,0 ), songToPlay)
		for i := 1; i < daySpan; i++ {
			callCowinUsingPin(pin, GetDate(loc, i), songToPlay)
		}
	...
	for usingDistrictID {
		callCowinUsingDid(did, GetDate(loc,0 ), songToPlay)
		for i := 1; i < daySpan; i++ {
			callCowinUsingDid(did, GetDate(loc, i), songToPlay)
		}
		time.Sleep(frequencyInMinutes * time.Minute)
	}
}
...
func callCowinUsingPin(pin, date, songToPlay string) {
	fmt.Printf("Results for %s\n", date)
	url := "https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/calendarByPin?pincode=" + pin + "&date=" + date
	callCowin(url, songToPlay)
}

func callCowinUsingDid(did, date, songToPlay string) {
	fmt.Printf("Results for  %s\n", date)
...
func callCowin(url, songToPlay string) {
	method := "GET"
